...
public class GaiaLink {
    ...
    private void listenBluetooth() throws IOException {
        if (mDebug)
            LogHelper.i(TAG, "listenBluetooth");
    ...
    private void connectBluetooth(BluetoothDevice device) {
        ...
        if (!getBluetoothAvailable()) {
            ...
            if (mDebug)
                LogHelper.i(TAG, "connect BT " + device.getAddress());
    ...
    private BluetoothSocket createSocket(UUID uuid) {
        ...
        try {
            ...
            if (mDebug)
                LogHelper.w(TAG, "createSocket: " + e.toString());
    ...
    private void connectUdp(String address) {
        ...
        try {
            ...
            if (mDebug)
                LogHelper.i(TAG, "connect UDP " + address);
    ...
    private void disconnectUdp() {
        if (mDebug)
            LogHelper.i(TAG, "disconnect UDP");
    ...
    private void sendCommandData(byte[] data, int commandId) {
        if (!mDfuBusy) {
            ...
            case BT_GAIA:
                ...
                if (mDebug)
                    LogHelper.i(TAG, "send command 0x" + Gaia.hexw(commandId));
                if(mBTSocket!=null){
                    try {
    ...
    private class Reader extends Thread {
        ...
        public void run() {
            ...
            if (mReceiveHandler == null)
                LogHelper.e(TAG, "reader: no receive handler");
        ...
        private void runSppReader() {
            ...
            LogHelper.i(TAG, "runSppReader start...");
            ...
            while (going) {
                try {
                    ...
                    if (mDebug)
                        LogHelper.e(TAG, "runSppReader: read: " + e.toString());
                    going = false;
                }
            }
        ...
        private void runUdpReader() {
            ...
            if (mReceiveHandler == null) {
                if (mDebug)
                    LogHelper.e(TAG, "No receive_handler");
            }
                ...
                try {
                    ...
                    if (mDebug)
                        LogHelper.i(TAG, "rx skt on " + GW_PORT_IN);
                    going = true;
                }

                catch (Exception e) {
                    if (mDebug)
                        LogHelper.e(TAG, "runUdpReader: " + e.toString());
                    e.printStackTrace();
                    // going = false;
                }
            ...
            for (int i = 0; i < length; ++i) {
                if ((packet_length > 0) && (packet_length < Gaia.MAX_PACKET)) {
                    ...
                    if (packet_length == Gaia.OFFS_FLAGS)
                        ...
                        if (mVerbose)
                            LogHelper.d(TAG, "expect " + expected);
                    }
                    ...
                    if (packet_length == expected) {
                        if (mVerbose)
                            LogHelper.d(TAG, "got " + expected);

                        if (mReceiveHandler == null) {
                            if (mDebug)
                                LogHelper.e(TAG, "No receiver");
                        }
                            ...
                            if (command.getEvent() == Gaia.EventId.START && !mIsConnected) {
                                if (mDebug)
                                    LogHelper.i(TAG, "connection starts");
                                ...
                                if (mDebug)
                                    LogHelper.i(TAG, "received command 0x" + Gaia.hexw(command.getCommand()));
    ...
    private void handleException(String name, GaiaError.TypeException type, Exception exception) {
        if (mDebug) {
            LogHelper.e(TAG, name + ": " + exception.toString());
        }
        if (mReceiveHandler != null) {
