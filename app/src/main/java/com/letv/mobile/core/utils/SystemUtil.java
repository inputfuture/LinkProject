...
public class SystemUtil {
    ...
    public static String getMacAddress() {// get mac from share
        ...
        String result = getMacAddressFromWifiManager();

        if (StringUtils.equalsNull(result)) {
            result = getSystemProperty(LOCALMAC);
        }
        if (StringUtils.equalsNull(result)) {
            ...
            if (TextUtils.isEmpty(result)) {
                ...
                if (!StringUtils.equalsNull(result) && result.length() > 0) {
                    result = result.substring(result.indexOf(HWADDR) + 6,
                            result.length() - 1);
                }
            }
        }
    ...
    public static boolean isAppRunningForeground() {
        ...
        if (Build.VERSION.SDK_INT >= 21) {
            List<RunningAppProcessInfo> processes = am
                    .getRunningAppProcesses();
    ...
    /**
     * 是否拥有系统属性；is has systemFeature of the key
     * @param key
     * @return
     */
    public static boolean hasSystemFeature(String key) {
        boolean isHasFeature = false;
        if (!StringUtils.equalsNull(key)) {
            isHasFeature = ContextProvider.getApplicationContext()
                    .getPackageManager().hasSystemFeature(key);
        }
        return isHasFeature;
    }
    /**
