package com.letv.leauto.ecolink.lemap.navi;
...
public class Utils {
    ...
    private static DecimalFormat sixnum = new DecimalFormat("#0.###");
    private static DecimalFormat twonum=new DecimalFormat("#0.##");
    ...
    public static String getFriendlyDistance2(int m) {
        float dis = m / 1000f;
        String disDes = fnum.format(dis);
        return disDes;
    }
    ...
    public static String getDistance(LatLng latLng,LatLng latLng2){
       float distance= AMapUtils.calculateLineDistance(latLng,latLng2);
        if (distance>1000){
            return  twonum.format(distance/1000f)+"千米";
        }else{
    ...
    public static double getSixDouble(double num){
        String result=sixnum.format(num);
